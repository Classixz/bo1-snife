...
init()
{
    ...
    PrecacheMenu("dukip_map_selection1");
    //call this when your match 'ends'
    level thread onPlayerConnect();
    level waittill("gm_snife_selected");
...
startMapVoting()
{
    ...
    for(i = 0; i < players.size; i++)
    {
        ...
        wait 0.05;
        players[i] OpenMenu("dukip_map_selection1");
    }
    wait 10;
...
generateTheMaps()
{
    mapList = [];
    mapList[0] = StrTok("mp_hanoi mp_firingrange mp_nuked", " ");
    mapList[1] = StrTok("mp_cairo mp_duga mp_crisis", " ");
    mapList[2] = StrTok("mp_russianbase mp_cosmodrome mp_cairo", " ");
    mapList[3] = StrTok("mp_cosmodrome mp_radiation mp_firingrange", " ");
    mapList[4] = StrTok("mp_nuked mp_villa mp_cairo", " ");
    currentMap = GetDvar("mapname");
    filteredList = removeExistingMapGroup(mapList, currentMap);
    goodList = StrTok(filteredList[RandomInt(filteredList.size)], " ");
    return goodList;
}
