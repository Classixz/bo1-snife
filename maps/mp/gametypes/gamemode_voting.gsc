...
init()
{
    ...
    PrecacheMenu("dukip_gm_selection");
    //call this when your match 'ends'
    level thread onPlayerConnect();
    level waittill("knife_won");
...
startMapVoting()
{
    ...
    {
        players[i] OpenMenu("dukip_gm_selection");
    }
    wait 10;
    ...
    if(winner == "dm_knife")
        mapIGN = "Free-For-All";
    else if(winner == "ctf_knife")
        mapIGN = "Capture the Flag";
    else if(winner == "sd_knife")
        mapIGN = "Search and Destroy";

    IPrintLnBold(mapIGN + " won!");

    wait 3;
    level notify("gm_knife_selected");
}
...
{
    gmList = [];
    gmList[0] = StrTok("dm_knife ctf_knife sd_knife", " ");//dm dom sd koth ctf dem
