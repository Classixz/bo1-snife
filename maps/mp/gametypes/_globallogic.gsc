...
endGame( winner, endReasonText )
{
	...
	level notify("stop_anticamp");
	//level waittill("new_map_selected"); before end game popup of winner/loser
	...
	level notify("select_snife");
	level waittill("new_map_selected");// YES! Right before loads up scoreboard, after Final Killcam
	//setDvar("sv_maprotation", "map " + level.winner_map);
	//setDvar("sv_maprotationcurrent", "map " + level.winner_map);
...
Callback_StartGameType()
{
	...
	if ( !isDefined( game["gamestarted"] ) )
	{
		...
		game["strings"]["team_up"] = &"PATCH_MP_TEAM_UP_PERIOD";
	...
	level thread teamchangeGracePeriod();
...
teamchangeGracePeriod()
{
	level endon( "game_ended" );
	if ( IsDefined( game["teamchange_graceperiod_done"] ) || isPregame() || level.allow_teamchange == "0" || level.wagermatch || level.gametype == "dm" )
	{
	...
	if( level.teamchange_graceperiod )
	{
		...
		wait( 10 );
		IPrintLn( game["strings"]["team_up"], level.teamchange_graceperiod );
		
		level.teamchange_graceperiod--;		
		
		while( level.teamchange_graceperiod > 0 )
		{
			wait( 1 );
			
			if( level.teamchange_graceperiod % 10 == 0 && level.teamchange_graceperiod >= 10 )
			{
				IPrintLn( game["strings"]["team_up"], level.teamchange_graceperiod );
			}
			
			if( level.teamchange_graceperiod % balanceModulus == 0 && level.teamchange_graceperiod >= balanceModulus )
			{
	...
	level.teamchange_graceperiod = 0;
	game["teamchange_graceperiod_done"] = true;
}